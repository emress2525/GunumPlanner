...
public class WebResearchTest {
    ...
    public void parsesDuckDuckGoResultsAndUnwrapsRealUrls() {
        ...
        List<WebSearchResult> results = DuckDuckGoSearch.parseResults(html, 5);
    ...
    @Test
    public void researchContextContainsEvidenceAndSourceUrls() {
        WebSearchResult result = new WebSearchResult("Prompt apps", "https://example.org/prompts", "Feature comparison");
        String context = ResearchContextBuilder.build("en iyi prompt yazıcı uygulamalarını araştır", java.util.Collections.singletonList(result));
        assertTrue(context.contains("LIVE WEB SEARCH RESULTS"));
